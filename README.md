...
| 테스트 소재 (Meta) | 광고세트명에 test가 든 세트의 소재 판정 화면. **판정 추천**(D+·지출·ROAS·구매 기준, [기준]에서 조정, '▶ 판정 필요' 칩·타일), 정렬 안 고르면 판정 필요 순, 행 썸네일(클릭=미리보기), **상품별 묶기**(소계·상태 수), 우리 대시보드로 만든 광고는 소구점·등록자·문구 연결, 추가소재 요청 → [소재 등록하러] 바로 이동. |
| 베스트 소재 (Meta) | 우수 판정 소재 모음. 타일마다 누적 지출·구매·ROAS(테스트 소재 데이터), 순위 배지, **상품별 묶기**(소계·세트 빼기), ROAS/지출/최근 정렬·상품 칩·꺼진 소재 숨기기, 타일 버튼 **모델 광고로·문구 복사·소재 등록**, 테스트 소재에서 [우수] 시 **자동 담기**(스위치), 미리보기에 문구·버튼. |
