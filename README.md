...
| CS 주문조회 | **카카오 상담톡 답변용** ([cs-lookup.html](cs-lookup.html), 초대코드 로그인). 전화번호·주문번호·주문자 이름 → 카페24 최근 3개월 주문·품목 상태·송장(네이버 배송조회 링크)·받는분·배송메모 + 자사몰 「🚚출고일 안내」 게시판(board 3001) 글에 상품이 있으면 배송지연 배지·해당 줄 표시 → 상태별 답변 문구 자동 생성·복사. 조회만 함(수정 없음). 서버: `supabase/functions/cs-lookup`(x-cs-code = CS_CODE secret, 카페24 토큰은 판매 성과와 공유), 배포 `./deploy-cs-lookup.sh <초대코드> <게시판번호>`. 게시판 읽기는 카페24 앱 권한 `mall.read_community` 추가 후 `cafe24-oauth?action=start` 재인증 필요. 답변 문구는 cs-lookup.html 상단 `R` 객체. |
