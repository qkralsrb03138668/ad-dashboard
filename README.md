...
| 소재 업로드 (구 광고소재 대시보드) | 소재 등록 + 상품 × 소재유형(스토리·썸네일릴스·릴스 …) 체크보드 — 칸 클릭으로 제작완료→진행중 기록, 진행률 요약, 유형 열 추가 가능 |
...
| 로그인 · 소재 등록 | **Supabase Auth 계정 + 역할(관리자/마케터)**. 마케터가 파일 드롭 → 파일명으로 카페24 상품 자동 매칭(중복 상품은 선택, 선택 기억) → URL 자동 → 문구 → Meta 보관함 업로드 + 서버 기록 → 체크보드 스토리/릴스 자동, 광고 업로드 탭에 자동 채움. 상품별 광고 문구 고정(product_copy): 저장본 자동 채움, [AI] 버튼으로 Claude(claude-opus-5) 생성·고정(`AI키-등록.command`). **셋업: [SETUP-로그인-소재등록.md](SETUP-로그인-소재등록.md)** |
| 테스트 소재 (Meta) | 광고세트명에 test가 든 세트의 소재 판정 화면. **판정 추천**(D+·지출·ROAS·구매 기준, [기준]에서 조정, '▶ 판정 필요' 칩·타일), 정렬 안 고르면 판정 필요 순, 행 썸네일(클릭=미리보기), **상품별 묶기**(소계·상태 수), 우리 대시보드로 만든 광고는 소구점·등록자·문구 연결, 추가소재 요청 → [소재 등록하러] 바로 이동. **전문 판정(2026-09-12)**: 상품 마진 **손익분기 ROAS**(OFF=미만·우수=×1.5↑, 카페24 판매가·공급가) + **표본 부족**(구매 5건 미만), **퍼널 진단** 열(노출→클릭→랜딩→장바구니→구매, 기준=테스트 소재 중앙값: 후크 약함/클릭 약함/랜딩 이탈/장바구니 이탈/클릭↑구매0/퍼널 균형), **추세 스파크라인**(일별 ROAS 막대·최근 7일 ROAS·식음 배지, test_ad_day 스냅샷). |
| 베스트 소재 (Meta) | 우수 판정 소재 모음. 타일마다 누적 지출·구매·ROAS(테스트 소재 데이터), 순위 배지, **상품별 묶기**(소계·세트 빼기), ROAS/지출/최근 정렬·상품 칩·꺼진 소재 숨기기, 타일 버튼 **모델 광고로·문구 복사·소재 등록**, 테스트 소재에서 [우수] 시 **자동 담기**(스위치), 미리보기에 문구·버튼. **순이익 등급(2026-09-12)**: 전환값×마진율×(1−순반품률 최근 60일)−지출 → S/A/B/C 배지·순이익 줄·순이익순 정렬·순이익 합 타일(관리자만, 판매 성과 netreturns) + 추세 스파크라인·식은 소재 타일. |
| 베스트 주간 리포트 | 베스트 소재 탭 [주간 리포트] — MD팀·컨텐츠팀 회의용. 기간 성과·전주 대비(서버가 매일 남기는 `test_ad_day` 스냅샷 기준 증분), 베스트 변동(새로 담김·꺼짐·TOP3·식음), 패턴(릴스/이미지·소구점·가격대·문구 훅), 상품 판단(확장/소재 추가 테스트/교체 검토), 테스트 효율(우수율·등록자별), 다음 주 액션. 텍스트 복사·PDF. **AI 해석**은 바탕화면 `주간리포트-해석.command`(scripts/weekly-insight.mjs, 이 맥의 Claude Code, 결제 없음)가 붙인다. |
