...
| 베스트 소재 (Meta) | 우수 판정 소재 모음. 타일마다 누적 지출·구매·ROAS(테스트 소재 데이터), 순위 배지, **상품별 묶기**(소계·세트 빼기), ROAS/지출/최근 정렬·상품 칩·꺼진 소재 숨기기, 타일 버튼 **모델 광고로·문구 복사·소재 등록**, 미리보기에 문구·버튼. **순이익 등급(2026-09-12)**: 전환값×마진율×(1−순반품률 최근 60일)−지출 → S/A/B/C 배지·순이익 줄·순이익순 정렬·순이익 합 타일(관리자만, 판매 성과 netreturns) + 추세 스파크라인·식은 소재 타일. |
