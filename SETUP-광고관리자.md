...
## 6-1. 23:55 세팅 열 = 시작 예산·23:55 원복 (2026-09-07 규칙 변경)
(2026-09-07 저녁) 용어 '자정'→'23:55'로 통일, **예약 실행 시각 00:00 → 23:55**. `schedule`은 23:55 전이면 apply_date=오늘, 지나면 내일. 23:55 크론(`run_reset`)이 **원복(예약 있는 세트 제외) → 예약 적용** 순으로 한 번에 실행하고, 00:00 크론(`run`)은 남은 예약분 적용용 보조로 남김(크론 스케줄 변경 불필요). 열 첫 줄에 `· 23:55 예약 우선` 표시. 다음날 시작 예산(00:10 스냅샷)은 예약 금액이 된다(=예약은 새 시작 예산).
사용자 규칙: "매일 처음 세팅된 일예산으로 23:55 자동 원복(당일 승인 시)". 열 첫 줄 `시작 N → 23:55 원복 / · 23:55 유지 / · 승인 전`(00:10 스냅샷 budget_daystart 기준, `pending` 응답의 daystart). ×10 기본값은 폐기 — 아래 입력칸+⏰는 자정 00:00에 따로 걸 금액(선택, 빈칸이면 예약 없음).
00:10을 놓친 날(도입일)은 `POST meta-budget?action=snapshot&backfill=1`(x-cron-secret)로 복원: 오늘 예산 이력의 첫 old_value, 변경 없던 세트는 현재값.
