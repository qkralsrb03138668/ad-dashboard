...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "products") {
      ...
      const rows: { product_no: number; name: string; image: string; price: number; supply_price: number; created: string }[] = [];
      // ponytail: 최대 20페이지(2,000개) 상한 — 몰 상품이 그보다 많아지면 offset 이어받기 파라미터 추가
      for (let offset = 0; offset < 2000; offset += 100) {
        const body = await apiGet(
          `${API_BASE}/admin/products?display=T&selling=T&fields=product_no,product_name,list_image,price,supply_price,created_date` +
        ...
        for (const pr of page) rows.push({
          ...
          created: String(pr.created_date ?? "").slice(0, 10),   // 카페24 상품 등록일 — 체크보드 '등록일' 열
        });
